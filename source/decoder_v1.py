...
# Set message clipping from 20 to 1000 to avoid problems with decoding at high SNR
...
class LDPCBPDecoder1(Layer):
    # pylint: disable=line-too-long
    r"""LDPCBPDecoder(pcm, trainable=False, cn_type='boxplus-phi', hard_out=True, track_exit=False, num_iter=20,
    stateful=False,output_dtype=tf.float32, **kwargs)
    ...
    (1) `boxplus`
        ...
        .. math::
            y_{j \to i} = 2 \operatorname{tanh}^{-1} \left( \prod_{i' \in \mathcal{N}_(j) \setminus i}
            \operatorname{tanh} \left( \frac{x_{i' \to j}}{2} \right) \right)

    (2) `boxplus-phi`

        .. math::
            y_{j \to i} = \alpha_{j \to i} \cdot \phi \left( \sum_{i' \in \mathcal{N}_(j) \setminus i} \phi
            \left( |x_{i' \to j}|\right) \right)
    ...
    .. math::
        y_{j \to i} = 2 \operatorname{tanh}^{-1} \left( \prod_{i' \in \mathcal{N}_(j) \setminus i} \operatorname{tanh}
        \left( \frac{\textcolor{red}{w_{i' \to j}} \cdot x_{i' \to j}}{2} \right) \right)
...
class LDPC5GDecoder1(LDPCBPDecoder1):
    # pylint: disable=line-too-long
    ...
    def __init__(self,
        ...
        self._llr_max = 10000  # internal max value for LLR initialization
